...
namespace FTN.Services.NetworkModelService.DataModel.Wires
{
    public class RegulatingControl : PowerSystemResource
    {
        ...
        public override void GetReferences(Dictionary<ModelCode, List<long>> references, TypeOfReference refType)
        {
            ...
            {
                references[ModelCode.REGULATINGCONDEQ_REGULATINGCONTROL] = regulatingCondEqs.GetRange(0, regulatingCondEqs.Count);
            }
            if (regulationSchedules != null && regulationSchedules.Count > 0 && (refType == TypeOfReference.Target || refType == TypeOfReference.Both))
            {
                references[ModelCode.REGULATIONSCHEDULE_REGULATINGCONTROL] = regulationSchedules.GetRange(0, regulationSchedules.Count);
            }
