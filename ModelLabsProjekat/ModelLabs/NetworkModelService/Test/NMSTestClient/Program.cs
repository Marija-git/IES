...
namespace FTN.Services.NetworkModelService.TestClient
{
	public class Program
	{
			...
			try
			{
				...
				do
				{
                    ...
                    else if (str == "6")
                    {
                        try
                        {
                            tgda.GetRegulatingControlsFromTerminalWithLowestGID();
                        }
                        catch (Exception ex)
                        {
		...
		private static void PrintMenu()
		{
            ...
            Console.WriteLine("\t5) Get all terminals");
            Console.WriteLine("\t6) Get Regulating Controls related to Terminal with lowest GID");
