...
namespace TelventDMS.Services.NetworkModelService.TestClient.Tests
{
	...
	{			
        ...
        {
            Console.WriteLine("Getting terminals values method started.");
            CommonTrace.WriteTrace(CommonTrace.TraceError, "Getting terminals values method started.");

            XmlTextWriter xmlWriter = null;
            int numberOfResources = 2;
            ModelCode modelCode = ModelCode.TERMINAL;

            try
            {
                List<ModelCode> propertyID = modelResourcesDesc.GetAllPropertyIds(modelCode);

                int iteratorId = GdaQueryProxy.GetExtentValues(modelCode, propertyID);
                int resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);

                xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\GetAllTerminals.xml", Encoding.Unicode);
                xmlWriter.Formatting = Formatting.Indented;
                ...
                while(resourcesLeft > 0)
                {
                    ...
                    {
                        resourceDesc[i].ExportToXml(xmlWriter);
                        xmlWriter.Flush();
                    }
                    resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);
                }
                GdaQueryProxy.IteratorClose(iteratorId);
                Console.WriteLine("Getting Terminals successfully finished!");
            }
            catch(Exception e)
            {
        ...
        public void GetRegulatingControlsFromTerminalWithLowestGID()
        {
            Console.WriteLine("Getting Regulating controls for Terminal with lowest gid method started.");
            CommonTrace.WriteTrace(CommonTrace.TraceError, "Getting Regulating controls for Terminal with lowest gid method started.");
            ...
            try
            {
                ...
                xmlWriter = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\RegulatingControlsForTerminalWithLowestGid.xml", Encoding.Unicode);
                xmlWriter.Formatting = Formatting.Indented;
            ...
            association.PropertyId = ModelCode.TERMINAL_REGULATINGCONTROLS;
            association.Type = ModelCode.REGULATINGCONTROL;
            ...
            try
            {
                ...
                GdaQueryProxy.IteratorClose(iteratorId);
                Console.WriteLine("Getting Regulating controls for Terminal with lowest gid method successfully finished!!");
            }
            catch(Exception e)
            {
                Console.WriteLine(e.Message);
